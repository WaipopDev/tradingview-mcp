...
def analyze_and_store_signal(
    ...
    if not isinstance(technical, dict) or "error" in technical:
        return {
            "stored": False,
            "error": technical.get("error") if isinstance(technical, dict) else "technical analysis failed",
            "symbol": symbol_clean,
    ...
    if not isinstance(mtf, dict) or "error" in mtf:
        return {
            "stored": False,
            "error": mtf.get("error") if isinstance(mtf, dict) else "multi-timeframe analysis failed",
            "symbol": symbol_clean,
            "exchange": exchange_clean,
            "timeframe": timeframe_clean,
        }
